...
totp = pyotp.TOTP(os.environ.get("TOTP_SECRET", ""), interval=30)
...
map_qdtype = dict(
    [(name, optimum.quanto.qtypes[name]) for name in optimum.quanto.qtypes]
    + [
        ("fp8", optimum.quanto.qfloat8),
        ("int8", optimum.quanto.qint8),
        ("int4", optimum.quanto.qint4),
        ("int2", optimum.quanto.qint2),
    ]
)
...
async def load_flux_model(config: dict) -> FluxPipeline:
    ...
    if "FluxTransformer2DModel" in pipeline:  # phased loading of models
        ...
        if "torch_dtype" in flux_transformer:
            flux_transformer["torch_dtype"] = getattr(
                torch, flux_transformer["torch_dtype"]
            )
    ...
    # Load/Run Options
    if (
        "enable_sequential_cpu_offload" in options
        and options["enable_sequential_cpu_offload"]
    ):
    ...
    if "transformer" in compile:
        ...
        flux_pipe.transformer = torch.compile(
            flux_pipe.transformer, mode="max-autotune", fullgraph=True
        )
    if "vae" in compile:
        logger.info(f"Torch compiling vae ...")
        flux_pipe.vae.to(memory_format=torch.channels_last)
        flux_pipe.vae = torch.compile(
            flux_pipe.vae, mode="max-autotune", fullgraph=True
        )
...
def config_loader(file_path: str, model: str = "dall-e-2") -> tuple:
    ...
    if not model in config["models"]:
        raise openedai.BadRequestError(
            f"Model not found in config: {model}", param="model"
        )
...
def load_generation_config(request: GenerationsRequest) -> tuple:
    ...
    generator_name, generator, enhancer = config_loader(
        args.config, model=request.model
    )
...
async def generate_images(pipe, **generation_kwargs) -> list:
    ...
    seed = (
        random_seed
        if random_seed != -1
        else int(time.time() * 1e6) & 0xFFFFFFFFFFFFFFFF
    )
...
async def generations(request: GenerationsRequest):
    resp = {"created": int(time.time() * 1000), "data": []}
    ...
    generator_name, model_config, generation_kwargs, enhancer = load_generation_config(
        request
    )
    ...
    if request.prompt.startswith(no_enhance_prompt):
        generation_kwargs["prompt"] = request.prompt = request.prompt[
            len(no_enhance_prompt) :
        ]
        enhancer = None

    if enhancer:
        try:
            generation_kwargs["prompt"] = revised_prompt = await enhance_prompt(
                generation_kwargs["prompt"], **enhancer
            )
    ...
    try:
        ...
        if images:
            for img in images:
                ...
                if request.response_format == "b64_json":
                    ...
                    img_dat = {
                        "url": presigned_url
                    }  # yeah it's lazy. requests.get() will not work with this, but web clients will
        ...
        logger.debug(
            f"Generated {len(images)} {request.model} image(s) in {time.time() - resp['created'] / 1000:.1f}s"
        )
...
def default_config_exists():
    if not os.path.exists(default_config_json):
        logger.info(
            f"Missing {default_config_json}, installing {default_config_template}"
        )
...
def parse_args(argv=None):
    ...
    parser.add_argument(
        ...
        default=None,
        type=int,
        help="The random seed to set for all generations. (default is random)",
    )
    parser.add_argument(
        "-L",
        "--log-level",
    ...
    parser.add_argument(
        "-P", "--port", action="store", default=5005, type=int, help="Server tcp port"
    )
    parser.add_argument(
        "-H",
        "--host",
        action="store",
        default="0.0.0.0",
        help="Host to listen on, Ex. 0.0.0.0",
    )
